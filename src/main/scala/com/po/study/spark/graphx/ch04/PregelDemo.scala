...
object PregelDemo extends App {
  /**
    ...
    *
    * (vid, vd, pd) => math.max(vd, pd) ##用mergeMsg传递来的消息更新节点属性
    * (sendMsg: EdgeTriplet[Int, String]) => Iterator((sendMsg.dstId, sendMsg.dstAttr + 1)) ##定义sendMsg及下一过程传递节点范围
    * (a, b) => math.max(a, b) ##定义消息合并处理逻辑
    */
  val pregel = Pregel(InitGraph.graph.mapVertices((vid, vd) => 0), 0, activeDirection = EdgeDirection.Out)((vid, vd, pd) => math.max(vd, pd), (sendMsg: EdgeTriplet[Int, String]) => Iterator((sendMsg.dstId, sendMsg.dstAttr + 1)), (a, b) => math.max(a, b))
